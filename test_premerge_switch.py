#!/usr/bin/env python3
"""
Test model switch with pre-merged weights.

Pre-merging qkv_proj and gate_up_proj during background loading
should speed up the injection phase significantly.
"""
...
def main():
    print("=" * 80)
    print("PRE-MERGE MODEL SWITCH TEST")
    print("=" * 80)
    ...
    # PHASE 1: Pre-load into arena (background)
    print("\n" + "=" * 80)
    print("PHASE 1: Pre-load into arena + pre-merge (background)")
    print("=" * 80)
    ...
    print(f"Tensor views: {view_time:.3f}s ({len(pinned_tensors)} tensors)")

    # Pre-merge (this is the new step - happens during background loading)
    print("Pre-merging weights for vLLM...")
    t0 = time.perf_counter()
    premerged_tensors = get_premerged_tensors_for_vllm(pinned_tensors)
    premerge_time = time.perf_counter() - t0
    print(f"Pre-merge: {premerge_time:.3f}s ({len(premerged_tensors)} tensors)")

    total_background_time = arena_load_time + view_time + premerge_time
    print(f"\nTotal background time: {total_background_time:.2f}s")

    # PHASE 2: Model switch (measured time)
    print("\n" + "=" * 80)
...
Background (can be hidden):
  - Arena load: {arena_load_time:.2f}s
  - Pre-merge: {premerge_time:.3f}s
  - Total: {total_background_time:.2f}s

Switch time (user-facing):
  - vLLM init with pre-merged weights: {switch_time:.2f}s

Comparison:
  - Baseline vLLM cold load: ~18s
  - Without pre-merge: ~18s
  - With pre-merge: {switch_time:.2f}s
  - Speedup: {18.27 / switch_time:.1f}x
""")
