#!/usr/bin/env python3
"""
Test vLLM with the new bulk pinned loading strategy.

Uses safetensors_load_strategy="pinned" to enable bulk pinned memory loading.
"""
...
def main():
    print("="*70)
    print("VLLM BULK PINNED LOADING TEST")
    print("="*70)
    ...
    # Config with pinned loading strategy
    config = {
        "dtype": "float16",
        ...
        "safetensors_load_strategy": "pinned",  # NEW: Enable bulk pinned loading
    }
    ...
    print("\n" + "-"*70)
    print("PHASE 1: Initialize Qwen (pinned loading)")
    print("-"*70)
    ...
    # Switch to Mistral
    print("\n" + "-"*70)
    print("PHASE 2: Switch Qwen -> Mistral (pinned loading)")
    print("-"*70)
    ...
    # Switch back to Qwen
    print("\n" + "-"*70)
    print("PHASE 3: Switch Mistral -> Qwen (pinned loading)")
    print("-"*70)
    ...
    if switch_results:
        ...
        print(f"\nAverage switch time (pinned): {avg:.0f}ms")

        # Compare to baseline
        baseline = 6597  # From baseline_switch.py
        improvement = (baseline - avg) / baseline * 100
        print(f"Baseline (no pinned): {baseline}ms")
        if improvement > 0:
