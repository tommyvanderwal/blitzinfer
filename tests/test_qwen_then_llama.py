#!/usr/bin/env python3
"""Test loading Qwen first, then llama - to see if it's gpt-oss specific."""
...
CRASH_LOG = os.path.expanduser('~/qwen_then_llama.log')
...
def main():
    ...
    # Load QWEN (NOT gpt-oss)
    crash_log("Loading qwen3-32b-fp8...")
    crash_log("LLM() constructor starting for qwen")
    llm = LLM(
    ...
    crash_log("LLM() constructor done for qwen")

    # Quick inference
    crash_log("Inference on qwen...")
    outputs = llm.generate(["Hello"], SamplingParams(max_tokens=5))
    crash_log(f"Output: {outputs[0].outputs[0].text[:50]}")

    # Prefetch qwen (simulating server behavior)
    crash_log("Starting prefetch of qwen (simulating server behavior)...")
    standby.start_prefetch("Qwen/Qwen3-32B-FP8")
    time.sleep(3)
    crash_log(f"Prefetch state: {standby.get_state().name}")

    # Cleanup qwen
    crash_log("Cleanup qwen starting...")
    ...
    # Now load llama
    crash_log("=" * 50)
    crash_log("LOADING LLAMA after QWEN (not gpt-oss)")
    crash_log("=" * 50)
    ...
    crash_log("SUCCESS - llama loaded after QWEN without crash!")
